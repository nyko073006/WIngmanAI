-- Add session-level search filter params to get_discover_profiles.
-- Swift passes these from SearchSettingsSheet; they override the user's profile defaults
-- for the current session without writing to the profiles table.

DROP FUNCTION IF EXISTS public.get_discover_profiles(text, text, text, text);
...
CREATE OR REPLACE FUNCTION public.get_discover_profiles(
  ...
  p_distance_km        text DEFAULT NULL,   -- '9999' = unlimited
  p_looking_for_filter text DEFAULT NULL,   -- '_all_' = no filter
  p_interested_in      text DEFAULT NULL    -- '_all_' = no filter, or comma-sep e.g. "Frauen,Männer"
)
RETURNS TABLE (
  user_id           uuid,
...
BEGIN
  ...
  IF p_distance_km IS NOT NULL THEN
    v_distance_km := p_distance_km::int;
  END IF;
  ...
  -- Relaxed mode: widen age ±2, distance +50 % (cap 100 km)
