-- Fix gender filter: profiles.gender stores 'male'/'female'/'diverse' (English)
-- but interested_in_arr stores 'women'/'men'/'divers' etc.
-- Normalize ALL interested_in values → 'male'/'female'/'diverse' to match profiles.gender.
...
BEGIN
  ...
  -- Normalize ALL interested_in values → English ('male'/'female'/'diverse')
  -- to match what profiles.gender actually stores.
  -- Input can be German UI labels, old German DB values, or English values.
  IF v_interested_in IS NOT NULL THEN
    v_interested_in := ARRAY(
      SELECT CASE lower(i)
        WHEN 'women'    THEN 'female'
        WHEN 'frauen'   THEN 'female'
        WHEN 'weiblich' THEN 'female'
        WHEN 'female'   THEN 'female'
        WHEN 'men'      THEN 'male'
        WHEN 'männer'   THEN 'male'
        WHEN 'männlich' THEN 'male'
        WHEN 'male'     THEN 'male'
        WHEN 'divers'   THEN 'diverse'
        WHEN 'diverse'  THEN 'diverse'
        ELSE NULL  -- drop unknown / 'alle' / 'all'
      END
      FROM unnest(v_interested_in) AS i
      WHERE lower(i) NOT IN ('alle', 'all')
    );
    -- Remove NULLs (from ELSE NULL above)
    v_interested_in := ARRAY(
      SELECT x FROM unnest(v_interested_in) AS x WHERE x IS NOT NULL
    );
